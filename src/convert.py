...
def convert_to_tflite(saved_model_dir, convert_name="model.tflite", same_name=False):
    ...
    if not os.path.exists(os.path.join(os.getcwd(), "converted_model")):
        os.mkdir(os.path.join(os.getcwd(), "converted_model"))
    ...
    with open(
        os.path.join(
            os.getcwd(),
            "converted_model",
            os.path.basename(saved_model_dir) + "_labels.txt",
        ),
        "w",
    ) as f:
        f.write("\n".join(["BrownSpot", "Healthy", "Hispa", "LeafBlast"]))
